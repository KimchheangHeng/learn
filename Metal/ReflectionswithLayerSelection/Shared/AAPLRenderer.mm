...
- (void) loadAssetsWithMetalKitView:(nonnull MTKView*)mtkView
{
    ...
    NSURL *modelFileURL = [[NSBundle mainBundle]
                           URLForResource: @"Models/Temple.obj"
                           withExtension:  nil];
    ...
    if(!sphereMeshes || error)
    {
        NSLog (@"Could not create sphere meshes");
    }
    ...
    if(!groundMeshes || error)
    {
        NSLog (@"Could not create ground meshes");
    }
...
- (void) updateGameState
{
    ...
    // We update the probe viewports :
    {
        ...
        for(int i = 0; i < 6; i++)
        {
            ...
            //    vertex shader to translate and project tne actors
    ...
    //  Peform culling and determine how many instances we need to draw
    {
...
- (void)drawInMTKView:(nonnull MTKView *)view
{
    ...
    // Wait to ensure only MaxBuffersInFlight are getting proccessed by any stage in the
    // Metal pipeline (App, Metal, Drivers, GPU, etc)
    dispatch_semaphore_wait(_inFlightSemaphore, DISPATCH_TIME_FOREVER);

    // Update the location(s) to which we'll write to in our dynamically changing Metal
    // buffers for the current frame (i.e. update our slot in the ring buffer used for the
    // current frame)
    ...
    {
        // Obtain the renderpass descriptor as late as possible; after updating any buffer state
        //   and rendering any previous passes.
        // By doing this as late as possible, we we don't hold onto the drawable any longer
        //   than necessary which could otherwise reduce our framerate as our application, the GPU,
        //   and display all contend for these limited drawables.
