...
struct Camera
{
    ...
    {
        return matrix_look_at (position, target, (vector_float3){0,1,0});
    }
...
struct CameraProbe
{
    ...
    matrix_float4x4 GetViewMatrixForFace_LH (int faceIdx) const
    {
        ...
        return matrix_look_at(position, position + directions[faceIdx], ups[faceIdx]);
    }
...
// Utility structure to test instersection between a frustum and parametric shapes
struct FrustumCuller
{
                    ...
                    const float           farPlaneDistance )
    {
        ...
        // we do the same, to get the top plane normal, fron the bottom plane :
...
@interface AAPLActorData : NSObject
    ...
    // mutliplier used in shading to color actors using the same mesh differently
