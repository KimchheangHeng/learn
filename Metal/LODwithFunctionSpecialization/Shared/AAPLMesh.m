...
/// Create a metal texture with the given semantic in the given ModelIO material object
                          ...
                          metalKitTextureLoader:(nonnull MTKTextureLoader *)textureLoader
{
    ...
    if(self)
    {
        ...
        _textures = [[NSMutableArray alloc] initWithCapacity:kNumMeshTextureIndices];

        // Fill up our texture array with null objects so that we can fill it by indexing into it
        for(NSUInteger shaderIndex = 0; shaderIndex < kNumMeshTextureIndices; shaderIndex++) {
        ...
        _textures[kTextureIndexMetallic] =
            [AAPLSubmesh createMetalTextureFromMaterial:modelIOSubmesh.material
        ...
        _textures[kTextureIndexRoughness] =
        [AAPLSubmesh createMetalTextureFromMaterial:modelIOSubmesh.material
        ...
        _textures[kTextureIndexAmbientOcclusion] =
            [AAPLSubmesh createMetalTextureFromMaterial:modelIOSubmesh.material
...
- (AAPLFunctionConstantIndices)mapTextureBindPointToFunctionConstantIndex:(AAPLTextureIndices)textureIndex
{
    switch (textureIndex)
    {
        case kTextureIndexBaseColor:
                        ...
                        withGlobalMapWeight:(float)globalWeight
{
    ...
    {
        AAPLFunctionConstantIndices constantIndex =
...
/// Load the ModelIO mesh, including vertex data and submesh data which have index buffers and
                                       ...
                                       error:(NSError * __nullable * __nullable)error
{
    ...
    //   data.  In this case we created the ModelIO vertex descriptor so that the layout of the
    //   vertices in the ModelIO mesh match the layout of vertices our Metal render pipeline
    ...
    //   changes some 32-bit floats into 16-bit floats or other types from which ModelIO cannot
    //   produce tangents
...
/// Traverses the ModelIO object hierarchy picking out ModelIO mesh objects and creating Metal
...
/// Uses ModelIO to load a model file at the given URL, create ModelIO vertex buffers, index buffers
///   and textures, applying the given ModelIO vertex descriptor to re-layout vertex attribute data
