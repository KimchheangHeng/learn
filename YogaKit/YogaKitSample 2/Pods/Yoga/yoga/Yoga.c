...
typedef struct YGStyle {
  YGDirection direction;
  YGFlexDirection flexDirection; //主轴的方向
  YGJustify justifyContent;//在主轴上的对齐方式
//defines how the browser distributes space between and around content items along the cross-axis of their container,
  YGAlign alignContent;
//defines how the browser distributes space between and around flex items along the cross-axis of their container.
//This means it works like justify-content but in the perpendicular direction.
  YGAlign alignItems;
  YGAlign alignSelf;//aligns flex items of the current flex line overriding the align-items value
  YGPositionType positionType;
  YGWrap flexWrap;//specifies whether flex items are forced into a single line or can be wrapped onto multiple lines
  YGOverflow overflow;
  YGDisplay display;
  float flex;
  float flexGrow;//定义项目的放大比例，默认为0，即如果存在剩余空间，也不放大//The flex grow factor of a flex item is relative to the size of the other children in the flex-container
  float flexShrink;//flex-shrink属性定义了项目的缩小比例，默认为1，即如果空间不足，该项目将缩小。
  YGValue flexBasis;//specifies the initial main size of a flex item. This property determines the size of the content-box unless specified otherwise using box-sizing.
...
typedef struct YGConfig {
  ...
  float pointScaleFactor; //一个 point 对应几个 pixel
  YGLogger logger;//log函数。。
  void *context;
} YGConfig;
