...
- (void)viewDidLoad
{
    ...
    // Listen when we are backgrounded (leaving the app) so we can exit edit mode.
    [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidEnterBackgroundNotification
                                                      object:nil
                                                       queue:nil
                                                  usingBlock:^(NSNotification *notification) {
                                                      // we are being backgrounded, exit edit mode in our table view
                                                      if (self.isEditing)
                                                      {
...
{
    UIAlertController *actionSheet = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"My Title", @"")
                                                                  message:NSLocalizedString(@"Select you Choice", @"")
                                                           preferredStyle:UIAlertControllerStyleActionSheet];
    
    UIAlertAction *mailButton = [UIAlertAction actionWithTitle:NSLocalizedString(@"Mail", @"")
                                                 style:UIAlertActionStyleDefault
                                               handler:^(UIAlertAction *action) {
                                                   NSArray *selectedPaths = [self.tableView indexPathsForSelectedRows];
                                                   if (selectedPaths) {} // mail action with the given selection
                                               }];
    UIAlertAction *addFavoritesButton = [UIAlertAction actionWithTitle:NSLocalizedString(@"Add to Favorites", @"")
                                                 style:UIAlertActionStyleDefault
                                               handler:^(UIAlertAction *action) {
                                                   NSArray *selectedPaths = [self.tableView indexPathsForSelectedRows];
                                                   if (selectedPaths) {} // add to favorites action with the given selection
                                               }];
    ...
    [actionSheet addAction:addFavoritesButton];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([[segue identifier] isEqualToString:@"showDetail"])
    {
        ...
        // find the target view controller
        MyViewController *destinationVC = (MyViewController *)navController.topViewController;
