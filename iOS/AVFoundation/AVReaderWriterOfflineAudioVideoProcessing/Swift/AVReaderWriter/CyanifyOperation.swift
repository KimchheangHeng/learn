...
class CyanifyOperation: Operation {
    ...
    fileprivate var sampleTransferError: Error?
	...
	override func start() {
		...
		asset.loadValuesAsynchronously(forKeys: ["tracks"]) {
			...
			do {
				...
				assetWriter = try AVAssetWriter(outputURL: self.outputURL, fileType: AVFileTypeQuickTimeMovie)
    ...
    */
	...
	fileprivate func makeReaderOutputsForTracks(_ tracks: [AVAssetTrack], availableMediaTypes: [String]) throws -> (videoReaderOutputs: [AVAssetReaderTrackOutput], passthroughReaderOutputs: [AVAssetReaderTrackOutput]) {
		// Decompress source video to 32ARGB.
		let videoDecompressionSettings: [String: AnyObject] = [
			String(kCVPixelBufferPixelFormatTypeKey): NSNumber(value: kCVPixelFormatType_32ARGB as UInt32),
	...
	fileprivate func makeVideoWriterInputsForVideoReaderOutputs(_ videoReaderOutputs: [AVAssetReaderTrackOutput]) throws -> [ReaderOutputAndWriterInput] {
	...
	fileprivate func makePassthroughWriterInputsForPassthroughReaderOutputs(_ passthroughReaderOutputs: [AVAssetReaderTrackOutput]) throws -> [ReaderOutputAndWriterInput] {
		/*
	...
	fileprivate func transferVideoTracks(_ videoReaderOutputsAndWriterInputs: [ReaderOutputAndWriterInput], group: DispatchGroup) {
	...
	fileprivate func transferPassthroughTracks(_ passthroughReaderOutputsAndWriterInputs: [ReaderOutputAndWriterInput], group: DispatchGroup) {
	...
	fileprivate func transferSamplesAsynchronouslyFromReaderOutput(_ readerOutput: AVAssetReaderOutput, toWriterInput writerInput: AVAssetWriterInput, onQueue queue: DispatchQueue, sampleBufferProcessor: ((_ sampleBuffer: CMSampleBuffer) throws -> Void)? = nil, completionHandler: @escaping (Void) -> Void) {
	...
	fileprivate func readingAndWritingDidFinish(_ assetReader: AVAssetReader, assetWriter: AVAssetWriter) {
		if isCancelled {
...
extension CVPixelBuffer {
    ...
    */
	func removeARGBColorComponentAtIndex(_ componentIndex: size_t) throws {
